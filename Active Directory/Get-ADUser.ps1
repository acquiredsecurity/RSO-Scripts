$scriptname = "GetADUser" # set a variable we will use later as the "ServerHost" and "LogFile"
function runfunc(
    ...
    $adusers = Get-ADUSer -Filter * -Properties Created, Enabled, Name, SID, DistinguishedName, PasswordLastSet, LastLogonDate, PasswordNeverExpires
    ForEach ($aduser in $adusers) {
        $logdataset.AddEvent(($aduser | select-object -Property Created, Enabled, Name, SID, DistinguishedName, PasswordLastSet, LastLogonDate, PasswordNeverExpires | Get-ObjectDatesToString -DateFormat "yyyy-MM-dd HH:mm:ss"))      
