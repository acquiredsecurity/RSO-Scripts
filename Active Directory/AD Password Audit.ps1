$scriptname = "AD-DomainPasswordPolicy" # set a variable we will use later as the "ServerHost" and "LogFile"
function runfunc(
    ...
    $ADPasswords = Get-ADDefaultDomainPasswordPolicy | Select ComplexityEnabled ,  DistinguishedName  , LockoutDuration , MaxPasswordAge , MinPasswordAge , MinPasswordLength  , PasswordHistoryCount , ReversibleEncryptionEnabled
    ForEach ($ADPassword in $ADPasswords) {
        $logdataset.AddEvent(($ADPassword | Select ComplexityEnabled ,  DistinguishedName  , LockoutDuration , MaxPasswordAge , MinPasswordAge , MinPasswordLength  , PasswordHistoryCount , ReversibleEncryptionEnabled | Get-ObjectDatesToString -DateFormat "yyyy-MM-dd HH:mm:ss"))      
